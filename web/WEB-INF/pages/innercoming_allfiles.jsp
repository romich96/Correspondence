...
<html>
<head>
    <title>Електронна кореспонденція: внутрішні файли</title>
    ...
    <div style="margin-left:30px; margin-right: 30px;">
        <nav class="navbar navbar-default">
            <ul class="nav nav-pills">
                ...
                <li><a href="/outcoming_allfiles">Вихідні файли</a></li>
                <li class="active"><a href="/innercoming_allfiles">Внутрішні файли</a></li>
            </ul>
        </nav>
        <div>
            <% int i = 0;%>
            <table id="myTable" class="table table-hover table-bordered" cellspacing="0" width="100%">
                ...
                <tr>
                    ...
                    <td>Заголовок внутрішнього</td>
                </tr>
                </thead>
                <tbody>
                <c:forEach items="${innercoming_allfiles}" var="file">
                    <tr data-value="${file.id}">
                        ...
                        <td align="center"><a href="/download/inbox/${file.id}/${file.fileHash}"><span
                                class="glyphicon glyphicon-download"
                                style="color:blue;"></span> Скачати</a></td>
                        <td>${file.getInnercoming().innerDocRegNo}</td>
                        <td>
                            <fmt:formatDate value="${file.getInnercoming().innerDocRegDate}"
                                            type="date"
                                            pattern="dd.MM.yyyy"
                                            var="theFormattedinnerDocRegDate"/>
                                            ${theFormattedinnerDocRegDate}
                        </td>
                        <td>${file.getInnercoming().innerDocType}</td>
                        <td>${file.getInnercoming().innerDocTitle}</td>
                    </tr>
                </c:forEach>
                </tbody>
            </table>
