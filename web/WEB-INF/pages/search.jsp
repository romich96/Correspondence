...
<html>
<head>
  <title>Електронна кореспонденція: пошук у файлах</title>
  ...
  <nav id="inhead" class="navbar navbar-default navbar-static-top" style="margin-left: 30px; margin-right: 30px;">
    <div class="container-fluid">
      ...
      <ul class="nav navbar-nav">
        ...
        <li class="active"><a>Пошук</a></li>
        <form class="navbar-form navbar-left" role="search" action="/search" method="post">
          <div class="form-group">
            <input type="text" class="form-control" name="pattern" placeholder="Фраза у тексті файлів" title="* в кінці фрази для нечіткого пошуку" value="${pattern}">
          </div>
      ...
      <table id="myTable" class="table table-hover table-bordered" cellspacing="0" width="100%">
        ...
        <c:forEach items="${searchResults}" var="file">
          <tr>
            ...
            <td align="center"><a href="${file.fileRef}"><span
                    class="glyphicon glyphicon-download"
                    style="color:blue;"></span> Скачати</a></td>
            <td>${file.docNapryam}</td>
            <td>${file.docNumber}</td>
            <td>
              <fmt:formatDate value="${file.docDate}"
                              type="date"
                              pattern="dd.MM.yyyy"
